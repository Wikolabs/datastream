  ...
  content: {
    fr: {
      ...
      persuasion: {
        ...
        title: "Vos donnees sont riches. Vos pipelines saignent.",
        paragraphs: [
          { type: "pathos", text: "Jeudi 2h17 du matin, un alerte PagerDuty reveille votre data engineer. Le pipeline Snowflake a casse — une colonne renommee dans Salesforce a deux semaines, jamais propagee, qui faisait tomber silencieusement 40% des lignes depuis 12 jours. Le COO va decouvrir au standup que les chiffres du dashboard CFO sont faux depuis avant le board. Votre data engineer est seul, fatigue, et il va devoir refaire le backfill historique a la main pendant qu'il essaie de comprendre quelles autres tables ont ete corrompues. Il avait demande de l'aide il y a six mois. Vous lui aviez dit d'attendre le prochain hire." },
          { type: "logos", text: "Gartner estime que 80% des entreprises B2B vont depasser leur budget data engineering en 2026 a cause de la dette technique sur les pipelines. McKinsey a mesure que 60% du temps des data engineers est passe a debugger des pipelines casses plutot qu'a construire de nouveaux datasets a valeur business. Le marche des plateformes data low-code croit a 38% par an — non parce que c'est elegant, mais parce que recruter une equipe data senior coute desormais 1.2M$ par an minimum pour des resultats lents." },
          { type: "ethos", text: "Wikolabs construit des agents IA en production depuis 2023 pour des scale-ups B2B, family offices et fintechs reglementees. Nous avons brule nos doigts sur les memes problemes que vous : pipelines qui hallucinent, briefs ignores, dashboards desertes. DataStream est ce que nous avons construit pour nos propres clients exigeants avant de le proposer au marche." },
          { type: "solution", text: "Concretement : vous selectionnez vos sources parmi 40+ connecteurs natifs en interface visuelle. DataStream genere les DAGs, applique vos regles de validation declaratives a chaque run, et envoie une alerte Slack si une derive est detectee — avant que le dashboard ne mente. 99.97% d'uptime garanti, latence pipeline sous 2s, zero DevOps requis. Vos donnees arrivent propres dans Snowflake ou BigQuery chaque matin, et votre data engineer dort enfin." },
        ],
      },
    ...
    en: {
      langLabel: "EN",
      tagLabel: "AI pipeline · Real-time · Zero DevOps",
      taglines: ["Your data, clean.", "Delivered in real time.", "Without a data team."],
      taglineAccentIdx: 1,
      desc: "DataStream orchestrates your data pipeline end to end — ingestion, transformation, validation and delivery to your warehouse — automatically, with no code and no hires.",
      navLinks: [
      ...
      metrics: [
        ...
        { value: "99.97%", label: "guaranteed uptime" },
        { value: "<2s", label: "pipeline latency" },
        { value: "0", label: "DevOps required" },
      ],
      features: [
        { icon: "🔌", title: "40+ native connectors", desc: "PostgreSQL, Snowflake, BigQuery, S3, Salesforce, HubSpot, Stripe — plug your sources in a few clicks, without writing a line of code." },
        { icon: "🛡️", title: "Quality monitoring", desc: "Declarative validation rules applied to every run. Slack and PagerDuty alerts triggered automatically if a dataset drifts or fails." },
        { icon: "⚙️", title: "Automatic orchestration", desc: "DAGs auto-generated from your configuration. Scheduling, automatic retry and historical backfill included — Airflow without the complexity." },
      ],
      steps: [
        { num: "01", title: "Connect your sources", desc: "Pick your sources from 40+ available connectors. Configuration is done through a visual interface, no code required." },
        { num: "02", title: "Define your rules", desc: "Configure your SQL or Python transformations and quality rules. DataStream generates and orchestrates the DAGs automatically." },
        { num: "03", title: "Your data lands clean", desc: "Every run validates, transforms and delivers your data into your warehouse or data lake. Anomaly alerts, zero manual intervention." },
      ],
      persuasion: {
        sectionTag: "Why now",
        title: "Your data is rich. Your pipelines are bleeding.",
        paragraphs: [
          { type: "pathos", text: "Thursday 2:17am, a PagerDuty alert wakes your data engineer. The Snowflake pipeline broke — a column renamed in Salesforce two weeks ago, never propagated, silently dropping 40% of rows for 12 days. The COO will discover at standup that the CFO dashboard numbers have been wrong since before the board meeting. Your data engineer is alone, exhausted, and now has to manually backfill history while trying to figure out which other tables got corrupted. He asked for help six months ago. You told him to wait for the next hire." },
          { type: "logos", text: "Gartner estimates 80% of B2B companies will overrun their data engineering budget in 2026 due to pipeline technical debt. McKinsey measured that 60% of data engineers' time goes to debugging broken pipelines rather than building new business-value datasets. The low-code data platform market is growing at 38% per year — not because it's elegant, but because hiring a senior data team now costs $1.2M minimum per year for slow results." },
          { type: "ethos", text: "Wikolabs has been building production AI agents since 2023 for B2B scale-ups, family offices and regulated fintechs. We burned our fingers on the same problems you face: hallucinating pipelines, ignored briefs, abandoned dashboards. DataStream is what we built for our own demanding customers before bringing it to market." },
          { type: "solution", text: "Concretely: you select your sources from 40+ native connectors through a visual interface. DataStream generates the DAGs, applies your declarative validation rules to every run, and sends a Slack alert when drift is detected — before the dashboard lies. 99.97% guaranteed uptime, sub-2s pipeline latency, zero DevOps required. Your data lands clean in Snowflake or BigQuery every morning, and your data engineer finally sleeps." },
        ],
      },
      ctaTitle: "Your pipeline in production today",
      ctaDesc: "Setup in under 30 minutes. No DevOps required. First results on the first run.",
      ctaPrimary: "Book a call",
      ctaWhatsApp: "WhatsApp",
      ctaDemo: "Request a demo",
      ctaSoonBadge: "Soon",
      footerTagline: "Automated AI data pipeline for ambitious teams",
    },
  },
};
